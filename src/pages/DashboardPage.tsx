    ...
    <div className="dashboard-page">
      ...
      <div className="dashboard-grid">
        <section className="dashboard-card" onClick={() => navigate('/timetable')}>
          ...
          <AppIcon name="chevron-right" style={{ width: '16px', color: 'var(--text-soft)' }} />
        </section>
        ...
        <section style={{ marginTop: '32px' }}>
          <h3 style={{ fontSize: '12px', fontWeight: 700, color: 'var(--text-soft)', textTransform: 'uppercase', letterSpacing: '0.05em', marginBottom: '16px' }}>내 다가오는 일정</h3>
          <div className="dashboard-mini-list">
            {myUpcomingMeetings.length > 0 ? (
              myUpcomingMeetings.map(meeting => (
                <div key={meeting.id} className="mini-item">
                  <div className="mini-time">{meeting.start.toLocaleTimeString('ko-KR', { hour: '2-digit', minute: '2-digit', hour12: false })}</div>
              ...
              <p style={{ fontSize: '13px', color: 'var(--text-soft)', padding: '12px' }}>예정된 회의가 없습니다.</p>
            )}
          </div>
        </section>
      </div>
